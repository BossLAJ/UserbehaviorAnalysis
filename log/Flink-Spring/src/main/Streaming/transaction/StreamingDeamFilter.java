...
fliter
 */
public class StreamingDeamFilter {
    public static void main(String[] args) throws Exception {
        //获取flink运行环境
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        //获取数据源
        DataStreamSource<Long> text = env.addSource(new MyNooParalleSource());
        ...
        DataStream<Long> resultData = filterData.map(new MapFunction<Long, Long>() {
            public Long map(Long value) throws Exception {
                System.out.println("过滤之后收到数据:" + value);
                return value;
            }
        });
        ...
        String jobName = StreamingDeamFilter.class.getSimpleName();
        env.execute(jobName);
    }
}
